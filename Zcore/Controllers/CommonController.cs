...
namespace Zcore.Controllers
{
    public class CommonController<T> : AuthenticatedController where T: class, new()
    {
        ...
        public override async Task<IActionResult> GetOne(string authorization, long id)
        {
            var value = await base.GetOne(authorization, id);
            return value ?? Ok(await _logicService.GetOne(await CheckAuth(authorization), id));
        }

        public override async Task<IActionResult> Put(string authorization, long id, object value)
        {
        ...
        public override async Task<IActionResult> Delete(string authorization, long id)
        {
